...
"""
Script pour formater les fichiers des dimanches de l'Avent.
Pour chaque fichier sun_advent_XX.txt dans models/fr/Temporal/,
met à jour le titre pour "XXe Dimanche de l'Avent" et applique le format standard.
"""
...
# Template pour les dimanches de l'Avent
ADVENT_TEMPLATE = """#title: {title}
#degree: 1
#category:
#occ: D1
#con: D1
...
#office:
    ##common: Du temps de l'Avent. Ant. & Capitules propres.
    ##matins: 1 Noct., 9 Pss.
    ##laudes: Laudes I
    ##prime: Psaume 117
    ##terce:
...
def format_advent_file(filepath: Path) -> None:
    """Formatte un fichier de dimanche de l'Avent."""
    # Extraire le numéro du nom de fichier
    filename = filepath.name
    match = re.match(r'sun_advent_(\d+)\.txt', filename)
    if not match:
        print(f"Ignoré: {filename} (ne correspond pas au pattern)")
        return

    numero = int(match.group(1))
    # Gestion des ordinaux pour l'Avent (1ER, 2e, 3e, 4e)
    if numero == 1:
        title = "1ER DIMANCHE DE L'AVENT"
    else:
        title = f"{numero}e DIMANCHE DE L'AVENT"
    ...
    try:
        ...
        content = ""
        updated_content = ADVENT_TEMPLATE.format(title=title)
    ...
    lines = content.split('\n')
    updated_lines = []
    template_lines = ADVENT_TEMPLATE.format(title=title).split('\n')
...
def main():
    ...
    for filepath in TEMPORAL_DIR.glob('sun_advent_*.txt'):
        format_advent_file(filepath)

    print("Formatage des dimanches de l'Avent terminé.")
